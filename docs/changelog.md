...
## [1.30.0] - 2025-06-01
### **BREAKING CHANGE**: Link fields now store ObjectIds directly instead of DBRef objects
- Changed `Link` fields to store `ObjectId`s directly in the database instead of `DBRef` objects
- This improves MongoDB compatibility and can simplify queries and reduce storage slightly
- **BREAKING CHANGE**: Existing data with `Link` fields stored as `DBRef`s will require migration. See the migrations tutorial for guidance
- Querying linked documents by ID now uses direct field comparison (e.g., `MyDoc.link_field == object_id`) instead of `MyDoc.link_field.id == object_id`
